...
namespace AdPackSimCs
{
    ...
    public sealed partial class MainPage : Page
    {
...
#endif
        {
            ...
            {
#if DECIMAL
                fixed (decimal* first = packs) //&packs[0]
#elif DOUBLE
                fixed (double* first = packs) //&packs[0]
#elif FLOAT
                fixed (float* first = packs) //&packs[0]
#endif
                {
                    var current = first;
                    ...
                    {
#if DECIMAL
                        if (*current == -1m) continue;
#elif DOUBLE
                        if (*current == -1d) continue;
#elif FLOAT
                        ...
                        *current += returnPerDayPerPack;
                        if (*current >= packReturn)
                        {
#if DECIMAL
                            *current = -1m;
#elif DOUBLE
                ...
                #region Indexed pointer
                //fixed (double* pointer = packs)
                //{
                //    for (int j = 0; j < allPacks; j++)
                //    {
                //        if (pointer[j] == -1d) continue;
                //        total += returnPerDayPerPack;
                //        if (total >= packPrice && i < reinvestingDays && activePacks < packsLimit)
                //        {
                //            total -= packPrice;
                ...
                //            activePacks++;
                //        }
                //        pointer[j] += returnPerDayPerPack;
                //        if (pointer[j] >= packReturn)
                //        {
                //            pointer[j] = -1d;
                //            activePacks--;
                //        }
